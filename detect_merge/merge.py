...
def show_elements(org_img, eles, show=False, win_name='element', wait_key=0, shown_resize=None, line=2):
    color_map = {
        ...
        'ImageButton':(128, 128, 255),
        'Button':(58, 88, 120),
        'ImageView':(128, 128, 0),
        'RadioButton':(0, 128, 255),
        'CheckBox':(60, 128, 65),
        'EditText':(10, 128, 65),
        'RatingBar':(60, 128, 65),
        'Switch':(200, 18, 65),
        'TextView':(0, 128, 65),
        'VideoView':(0, 28, 65),
        'ToggleButton':(100, 128, 65),
        'Chronometer':(0, 128, 65),
        'Spinner':(80, 28, 65),
        'SeekBar':(0, 128, 255)
    }

#'Button', 'CheckBox', '', 'EditText', 'ImageButton', 'ImageView',
#                              'ProgressBar', 'RadioButton', 'RatingBar', 'SeekBar', 'Spinner', '',
#                              '', 'VideoView', 'TextView']
...
def refine_elements_all(compos, texts, intersection_bias=(2, 2), containment_ratio=0.8):
    '''
    return all comp and text
    '''
    elements = []
    contained_texts = []
    idx=0
...
def merge(img_path, compo_path, text_path, merge_root=None, is_paragraph=False, is_remove_bar=True, show=False, wait_key=0):
    ...
    for text in text_json['texts']:
        element = Element(ele_id, (text['column_min'], text['row_min'], text['column_max'], text['row_max']), 'Text', text_content=text['content'].replace(" ",""))
    ...
    board = show_elements(img_resize, texts + compos, show=show, win_name='all elements before merging', wait_key=wait_key)

    # refine elements
    texts = refine_texts(texts, compo_json['img_shape'])
    elements = refine_elements_all(compos, texts)

#    elements2 = refine_elements(compos, texts)
    if is_remove_bar:
    ...
    show_elements(img_resize, elements, show=show, win_name='elements after merging', wait_key=wait_key)
...
#    components = save_elements(pjoin(merge_root, name + '.json'), text, img_resize.shape)
