...
#    input_file = "./tmp.jpg"
#    cv2.imwrite(input_file, res_img)

#    ocr_result = ocr.ocr_detection_google(res_img)
...
#    print("[Text Detection Completed in %.3f s] Input: %s Output: %s" % (time.clock() - start, input_file, pjoin(ocr_root, name+'.json')))
    print("[Text Detection Completed ] Input: %s Output: %s" % (input_file, pjoin(ocr_root, name+'.json')))
